...
If not a clear high-confidence favourite bet: {{"signal": false, "direction": "SKIP", "reasoning": "brief"}}"""
    ...
    async def analyze_markets(self, markets: List[Dict]) -> List[Dict]:
        ...
        macro_candidates = [m for m in macro_markets if 0.08 <= m["yes_price"] <= 0.92 and (m.get("liquidity", 0) > 0 or m.get("volume", 0) > 0)]
        ...
        def resolves_soon(market):
            end = market.get('end_date', '')
            if not end:
                return False
            try:
                end_dt = datetime.fromisoformat(end.replace('Z', '+00:00'))
                return now <= end_dt <= soon
            except:
                return True  # If can't parse, include it
        
        sports_candidates = [
            m for m in sports_markets
            if (m.get("liquidity", 0) > 0 or m.get("volume", 0) > 0)
            and (m["yes_price"] >= 0.72 or m["yes_price"] <= 0.28)
            and resolves_soon(m)
        ]
        # If nothing resolves soon with liquidity, take any game with a favourite today
        if not sports_candidates:
            sports_candidates = [
                m for m in sports_markets
                if (m["yes_price"] >= 0.72 or m["yes_price"] <= 0.28)
                and resolves_soon(m)
            ]
        sports_candidates.sort(key=lambda x: abs(x['yes_price'] - 0.5), reverse=True)

        # Debug: show what fields sports markets actually have
        if sports_markets:
            sample = sports_markets[0]
            log.info(f"Sample sports market fields: ticker={sample.get('ticker','')} yes={sample.get('yes_price',0):.2f} liq={sample.get('liquidity',0)} vol={sample.get('volume',0)}")
        ...
        for m in sports_candidates[:3]:
            log.info(f"  Top sports: {m["ticker"]} yes={m["yes_price"]:.0%} liq={m.get("liquidity",0)} vol={m.get("volume",0)}")
