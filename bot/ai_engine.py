...
Signal requirements:
- Favourite at 75%+ ✓ (already confirmed)
- No major injuries or disruptions from live news
- Game resolves within 24 hours
- Volume > 0 confirms real trading activity

JSON only: {{"true_probability_yes": {yes_price + 0.05:.4f}, "market_price_yes": {yes_price:.4f}, "edge": 0.05, "direction": "BUY_{fav_side}", "confidence": "HIGH", "reasoning": "Favourite confirmed, no injury news found", "key_risk": "upset risk", "signal": true}}
Skip if injuries found or no confidence: {{"signal": false, "direction": "SKIP", "reasoning": "brief"}}"""
    ...
    async def analyze_markets(self, markets: List[Dict]) -> List[Dict]:
        ...
        macro_candidates = [
            ...
            and (m.get('liquidity', 0) > 0 or m.get('volume', 0) > 0)
        ]
        macro_candidates.sort(key=lambda x: x.get('liquidity', 0) + x.get('volume', 0), reverse=True)
        ...
        for m in sports_markets:
            ...
            if end:
                try:
                    end_dt = datetime.fromisoformat(end.replace('Z', '+00:00'))
                    if not (now <= end_dt <= now + timedelta(hours=36)):
                        continue
                except:
                    pass
