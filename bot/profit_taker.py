...
class ProfitTaker:
    ...
    async def fetch_market_volume(self, ticker: str) -> float:
        """Fetch 24h volume for a market to check liquidity before selling."""
        session = await self._get_session()
        path = f'/trade-api/v2/markets/{ticker}'
        headers = get_auth_headers('GET', path)
        try:
            async with session.get(
                ...
                data = await resp.json()
                market = data.get('market', {})
                return float(market.get('volume_24h_fp') or market.get('volume_fp') or 0)
        except:
            return 0
    ...
    async def place_sell_order(self, ticker: str, side: str,
        ...
        # Price must be provided for sell orders
        yes_price_cents = round(current_price * 100) if sell_side == 'yes' else round((1 - current_price) * 100)
    ...
    async def check_and_exit_positions(self, open_positions: Dict) -> List[Dict]:
        ...
        for ticker in tickers:
            ...
            if action:
                ...
                # Check volume BEFORE selling to avoid slippage on thin markets
                market_volume = await self.fetch_market_volume(ticker)
                min_volume = 50000 if market_type == 'macro' else 5000
                if market_volume < min_volume:
                    log.info(
                        f"[PROFIT_TAKER] ALERT — {action} triggered on {ticker} "
                        f"but volume {market_volume:.0f} too low (min {min_volume}). "
                        f"Profit: {desc}. Consider selling manually on Kalshi to avoid slippage."
                    )
                    continue
