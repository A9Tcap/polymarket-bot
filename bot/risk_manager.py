    ...
    def _evaluate_opportunity(self, opp: Dict) -> Dict:
        ...
        if direction in ('BUY_YES', 'BUY_NO'):
            ...
            if bet_price < self.min_probability:
                log.info(f"Skipping: price {bet_price:.2%} below min probability {self.min_probability:.2%}")
                return None
            if bet_price > self.max_probability:
                log.info(f"Skipping: price {bet_price:.2%} above max probability {self.max_probability:.2%}")
                return None
        ...
        if source == 'sports_engine':
            # Flat betting for sports heavy favourites — Kelly doesn't work well
            # on low-price NO bets. Use fixed 1% of bankroll.
            position_size = self.bankroll * 0.01
        else:
