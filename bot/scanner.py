...
All series fetched in parallel so a single timeout never blocks the whole scan.
"""
...
SPORTS_SERIES = [
    ...
    'KXEPLGAME', 'KXLALIGAGAME', 'KXSERIEAGAME', 'KXUCLGAME',
]
...
async def fetch_one_series(session, series_ticker, limit, market_type):
    """Fetch a single series with timeout. Returns (series_ticker, market_type, markets)."""
...
class MarketScanner:
    ...
    async def get_active_markets(self, min_liquidity=0):
        ...
        # Run all concurrently with a global 15s timeout
        try:
