            ...
            while True:
                ...
                for market in batch:
                    ...
                    if len(sample_tickers) < 20:
                        sample_tickers.append(f"{ticker}(liq={market.get('liquidity',0)})")
                    
                    if series in GOOD_SERIES:
                        matched_series.add(series)
                        parsed = self._parse_market(market)
                        if parsed:
                            all_markets.append(parsed)

                cursor = data.get('cursor')
                if not cursor or total_seen >= 5000:
                    break

        except Exception as e:
            log.error(f"Error fetching markets: {e}", exc_info=True)

        log.info(f"Scanned {total_seen} markets total")
        log.info(f"Sample tickers: {sample_tickers[:10]}")
        log.info(f"Matched series: {matched_series}")
