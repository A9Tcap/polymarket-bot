...
# Kalshi economics/politics/crypto ticker prefixes
# These are the series prefixes for non-sports markets
GOOD_TICKER_PREFIXES = [
    'KXCPI', 'KXFED', 'KXGDP', 'KXINX', 'KXUNEM', 'KXPCE',
    'KXBTC', 'KXETH', 'KXCRYPTO', 'KXNQ', 'KXSPY', 'KXDOW',
    'KXTRUMP', 'KXPRES', 'KXCONG', 'KXSEN', 'KXHOUSE', 'KXGOV',
    'KXOIL', 'KXGOLD', 'KXDXY', 'KXEUR', 'KXJPY',
    'KXREC', 'KXHIGH', 'KXLOW', 'KXTARIFF', 'KXIRAN',
    'KXAI', 'KXTECH', 'KXCLIMATE', 'KXWEATHER', 'KXTEMP',
]
...
def is_good_ticker(ticker):
    ...
    for prefix in GOOD_TICKER_PREFIXES:
...
class MarketScanner:
    ...
    async def get_active_markets(self, min_liquidity=100):
        ...
        for m in all_markets[:10]:
            log.info(f"  Market: [{m['ticker']}] {m['question'][:70]}")
