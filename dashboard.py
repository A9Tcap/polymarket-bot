...
function render(data) {
  ...
  console.log('Kalshi data:', JSON.stringify(data.kalshi));
  ...
  document.getElementById('stats').innerHTML = `
    ...
    <div class="stat-card blue">
      ...
      <div class="stat-sub">${kalshi.positions && kalshi.positions.length > 0 ? kalshi.positions.map(p => p.side + ' ' + p.contracts + ' ' + p.ticker.split('-')[0]).join(' | ') : 'none'}</div>
    </div>
    <div class="stat-card blue">
    ...
    <div class="stat-card ${realPnl != null ? (realPnl >= 0 ? 'green' : 'red') : 'grey'}">
      <div class="stat-label">ROI</div>
      <div class="stat-value ${realPnlClass}">${realPnl != null ? (realPnl >= 0 ? '+' : '') + '$' + Math.abs(realPnl).toFixed(2) : 'loading...'}</div>
      <div class="stat-sub">${realPnl != null ? ((realPnl / startingBalance) * 100).toFixed(2) + '% vs $1,500 start' : 'waiting for Kalshi data'}</div>
    </div>
  `;
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(f"{base_url}{path}", headers=get_headers('GET', path),
                ...
                if resp.status == 200:
                    ...
                    # balance = cash only, positions_value = open position value
                    # Together they equal total portfolio (what Kalshi shows)
                    balance_cents = data.get('balance', 0)
                    positions_value = data.get('positions_value', 0) or 0
                    result['balance'] = (balance_cents + positions_value) / 100
                    result['cash'] = balance_cents / 100
                    result['positions_value'] = positions_value / 100
            ...
            async with session.get(f"{base_url}{path}", headers=get_headers('GET', path),
                ...
                if resp.status == 200:
                    ...
                    for pos in data.get('market_positions', []):
                        ...
                        # total_traded_fp is in contracts, resting_orders_count helps
                        # Use market_exposure or total_cost for value
                        cost = float(pos.get('total_cost', 0) or 0) / 100
                        exposure = float(pos.get('market_exposure', 0) or 0) / 100
                        if yes_pos != 0:
