...
function render(data) {
  ...
  document.getElementById('stats').innerHTML = `
    <div class="stat-card ${r.total_pnl >= 0 ? 'green' : 'red'}">
      <div class="stat-label">Total P&L</div>
      <div class="stat-value ${pnlClass}">${r.total_pnl >= 0 ? '+' : ''}$${r.total_pnl.toFixed(2)}</div>
      <div class="stat-sub">simulated USD</div>
    </div>
    <div class="stat-card ${parseFloat(roi) >= 0 ? 'green' : 'red'}">
      <div class="stat-label">ROI</div>
      <div class="stat-value ${roiClass}">${roi !== '—' ? (parseFloat(roi) >= 0 ? '+' : '') + roi + '%' : '—'}</div>
      <div class="stat-sub">return on staked</div>
    </div>
    ...
    <div class="stat-card yellow">
      ...
      <div class="stat-value neutral">$${r.total_staked.toFixed(0)}</div>
      <div class="stat-sub">${r.total_simulated} trades placed</div>
    </div>
    <div class="stat-card blue">
      <div class="stat-label">Pending</div>
      <div class="stat-value blue">${r.total_simulated - r.resolved}</div>
...
async def handle_api(request):
    ...
    return web.json_response({'results': results, 'trades': trades})
