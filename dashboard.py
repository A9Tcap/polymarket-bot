...
function render(data) {
  ...
  document.getElementById('stats').innerHTML = `
    ...
    <div class="stat-card ${realPnl != null ? (realPnl >= 0 ? 'green' : 'red') : 'red'}">
      <div class="stat-label">ROI</div>
      <div class="stat-value ${realPnlClass}">${realPnl != null ? (realPnl >= 0 ? '+' : '') + '$' + Math.abs(realPnl).toFixed(2) : '—'}</div>
      <div class="stat-sub">${realBalance != null ? ((realPnl / startingBalance) * 100).toFixed(2) + '% return' : '—'}</div>
    </div>
  `;
